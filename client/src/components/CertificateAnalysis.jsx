...
const CertificateAnalysis = (props) => {
  ...
  useEffect(() => {
    ...
    const getCertificate = async (obj)=>{
      const responseData = await fetch("http://localhost:4000/api/v1/analysis/certificate" ,{
          method:'POST',
          headers:{
  ...
  setNumberPacks(data?.number_of_packs)
    ...
    <StyledForm style={{paddingInline : 20}}>
      ...
      <div className="form-row">
        ...
        <div className="form-group col-md-4">
          ...
          <input type="text" className="form-control" id="Retest date" value={data?.updation_date==null?"null":data?.updation_date} readOnly />
        </div>
      </div>
      <div className="form-row">
        <div className="form-group col-md-4">
